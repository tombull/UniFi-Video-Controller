...
if [ -e "$confFile" ]; then
  ...
  datenow=$(date)
  echo "# unifi-video v3.10.13" > "$confFile"
  echo "#${datenow}" >> "$confFile"
  echo "is_default=false" >> "$confFile"
  echo "uuid=${uuid}" >> "$confFile"
  echo "# app.http.port = 7080" >> "$confFile"
  echo "# app.https.port = 7443" >> "$confFile"
  echo "# ems.liveflv.port = 6666" >> "$confFile"
  echo "# ems.livews.port = 7445" >> "$confFile"
  echo "# ems.livewss.port = 7446" >> "$confFile"
fi
...
if ! [[ -z "$UNIFI_VIDEO_LIVEFLV_PORT"  ]]; then
  settings["ems.liveflv.port"]="$UNIFI_VIDEO_LIVEFLV_PORT"
fi

if ! [[ -z "$UNIFI_VIDEO_LIVEWS_PORT"  ]]; then
  settings["ems.livews.port"]="$UNIFI_VIDEO_LIVEWS_PORT"
fi

if ! [[ -z "$UNIFI_VIDEO_LIVEWSS_PORT"  ]]; then
  settings["ems.livewss.port"]="$UNIFI_VIDEO_LIVEWSS_PORT"
fi
...
if [ "${BIND_PRIV}" == "true" ]; then
	if [ "$UNIFI_VIDEO_HTTP_PORT" -le "1024" ]; then
		touch /etc/authbind/byport/$UNIFI_VIDEO_HTTP_PORT
		chown unifi-video:unifi-video /etc/authbind/byport/$UNIFI_VIDEO_HTTP_PORT
		chmod 755 /etc/authbind/byport/$UNIFI_VIDEO_HTTP_PORT
	fi
	if [ "$UNIFI_VIDEO_HTTPS_PORT" -le "1024" ]; then
		touch /etc/authbind/byport/$UNIFI_VIDEO_HTTPS_PORT
		chown unifi-video:unifi-video /etc/authbind/byport/$UNIFI_VIDEO_HTTPS_PORT
		chmod 755 /etc/authbind/byport/$UNIFI_VIDEO_HTTPS_PORT
	fi
	if [ "$UNIFI_VIDEO_LIVEFLV_PORT" -le "1024" ]; then
		touch /etc/authbind/byport/$UNIFI_VIDEO_LIVEFLV_PORT
		chown unifi-video:unifi-video /etc/authbind/byport/$UNIFI_VIDEO_LIVEFLV_PORT
		chmod 755 /etc/authbind/byport/$UNIFI_VIDEO_LIVEFLV_PORT
	fi
	if [ "$UNIFI_VIDEO_LIVEWS_PORT" -le "1024" ]; then
		touch /etc/authbind/byport/$UNIFI_VIDEO_LIVEWS_PORT
		chown unifi-video:unifi-video /etc/authbind/byport/$UNIFI_VIDEO_LIVEWS_PORT
		chmod 755 /etc/authbind/byport/$UNIFI_VIDEO_LIVEWS_PORT
	fi
	if [ "$UNIFI_VIDEO_LIVEWSS_PORT" -le "1024" ]; then
		touch /etc/authbind/byport/$UNIFI_VIDEO_LIVEWSS_PORT
		chown unifi-video:unifi-video /etc/authbind/byport/$UNIFI_VIDEO_LIVEWSS_PORT
		chmod 755 /etc/authbind/byport/$UNIFI_VIDEO_LIVEWSS_PORT
	fi
fi
...
if authbind --deep /usr/sbin/unifi-video "${unifi_video_opts}" start; then
  echo "done."
else
  echo "failed."
  exit 1
